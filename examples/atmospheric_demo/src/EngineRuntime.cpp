...
std::unique_ptr<RuntimeMode> createLoadingTransition(const RuntimeTransitionRequest& request)
{
    ...
    std::unique_ptr<RuntimeMode> nextRuntimeMode = request.targetId == RuntimeId::VNPrototype
                                                        ...
                                                        : RuntimeFactory::create(request.targetId);
...
void EngineRuntime::processRuntimeTransition()
{
    ...
    activateRuntimeMode(createLoadingTransition(*requestedRuntimeChange));
}
} // namespace engine
